        ...
        if (contractObj.id !== undefined && contractObj.id !== null) {
            ...
            Object.entries(contractObj.contract).forEach(item => {
                if (Object.entries(defaultSections).reduce((prev, curr) => {return (prev && (item[0]!==curr[1].id))}, true)){
                   ...
                   // This is data for a default section
                   // we need to parse if the compensation is fixed fee or hourly wage
                    if (item[0] === "COMPENSATION"){
                        let fixedFee = ("fe-amount" in contractObj.contract.COMPENSATION || "fe-payment-buffer" in contractObj.contract.COMPENSATION);
                        if (fixedFee) copyDefaultElements["COMPENSATION"] = feCompensationElements;
                        else copyDefaultElements["COMPENSATION"] = hwCompensationElements;
                    }
